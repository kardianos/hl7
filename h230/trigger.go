...
//
// An A01 event is sent as a result of a patient undergoing the admission process which assigns the patient to a bed.  It signals
// the beginning of a patient’s stay in a healthcare facility.  Normally, this information is entered in the primary Patient
// Administration system and broadcast to the nursing units and ancillary systems.  It includes short stay and John Doe admissions.
type ADT_A01 struct {
...
// accordingly; the nursing system that the patient has been discharged and that the care plan can be completed; the finance
// system that the patient billing period has ended; and/or the clinical repository that discharge has taken place for the
// EMR.
//
...
// in preparation for a patient’s visit or stay in a healthcare facility.  For example, a preadmit may be performed prior to
...
//
// The A09 and A10 (patient arriving tracking) events are used when there is a change in a patient’s physical location and when
// this is NOT a change in the official census bed location.
//
...
//
// For “non-admitted” patients, the A11 event is sent when an A04 (register patient) event is canceled, either because of
...
// or because of a decision not to discharge or end the visit of the patient after all.  PV13-assigned location should reflect
// the location of the patient after the cancellation has been processed.  Note that this location may be different from the
// patient’s location prior to the erroneous discharge.  Prior Location could be used to show the location of the patient prior
// to the erroneous discharge.
type ADT_A13 struct {
...
//
// The A18 event is used to merge current and previous patient identification numbers:  PID-3-patient ID ( internal ID), PID-2-patient
// ID (external ID), PID-4-alternate patient ID-PID, and PID-18-patient account number.  This procedure is required, for
// example, when a previous patient  is registered under a new patient identification number because of an error, or because
// there was insufficient time to determine the actual patient identification number.  The merge event occurs when a decision
// is made to combine the information under either the new or the old identifier(s).  We recommend that events A34 (merge patient
// information-patient ID only), A35 (merge patient information-account number only), A36 (merge patient information-patient
// ID and account number), A39 (merge person-external ID), A40 (merge patient-internal ID), A41 (merge account-patient
// account number), and A42 (merge visit-visit number) be utilized in place of the A18 event whenever possible.
//
...
// For sites requiring (or desiring) greater specificity with regard to this type of message,  new events (A34 (merge patient
// information-patient ID only), A35 (merge patient information-account number only), A36 (merge patient information-patient
// ID & account number), A39 (merge person-external ID), A40 (merge patient-internal ID), A41 (merge account-patient account
...
// a link event, the affected patient data records should remain distinct.
type ADT_A24 struct {
...
// Patient Administration events.  Visit information may be included but is not required.  These events are primarily for
// demographic data, but optional historical non-demographic data may be sent as well.
type ADT_A28 struct {
...
// the information was performed in error, or when another record already exists for the person, or one wants to purge the person
// from the database.  When this event occurs, all visit and account level data for this person is also purged.
type ADT_A29 struct {
...
//
// An A30 event can be used to merge person information on an MPI.  The A34 (merge patient information-patient ID only), A35
// (merge patient information-account number only), A36 (merge patient information-patient ID & account number), A40
// (merge patient-internal ID), A41(merge account-patient account number) and A42 (merge visit-visit number)events
// should be used to merge patient information for a current episode.  The “incorrect MRN” identified on the MRG segment (MRG-1-prior
// patient ID-internal) is to be merged with the “correct MRN” identified on the PID segment (PID-3-patient ID (internal
// ID)).  The “incorrect MRN” then no longer exists.  All PID data associated with the “correct MRN” are treated as updated information.
//
...
//
// A merge has been done at the internal identifier level.  That is, two PID-3-patient ID (internal ID) identifiers have been
// merged into one.
type ADT_A40 struct {
...
//
// An A43 event is used to signal a move of records identified by the MRG-1-prior patient ID-Internal from the “incorrect source
// external identifier” identified in the MRG segment (MRG-4-prior patient ID-external) to the “correct target external
// identifier” identified in the PID segment (PID-2-patient ID-external ID).
// The internal ID involved in identifying the patient to be moved (MRG-1-prior patient ID-internal) may or may not have accounts,
// which may or may not have visits.
//
// A move has been done at the internal identifier level. That is, a PID-3-patient ID (internal ID) associated with one external
// identifier (PID-2-patient ID-external ID) has been moved to another external identifier.
type ADT_A43 struct {
...
// source internal identifier” identified in the MRG segment (MRG-1-prior patient IDinternal) to the “correct target internal
// identifier” identified in the PID segment (PID-3-patient ID internal ID).
//
// A move has been done at the account identifier level.  That is, a PID-18-patient ID (account number) associated with one
// internal identifier (PID-3-patient ID (internal ID) has been moved to another internal identifier.
type ADT_A44 struct {
...
//
// An A45 event is used to signal a move of records identified by the MRG-5-prior visit number or the MRG-6prior alternate visit
// ID  from the “incorrect source account identifier” identified in the MRG segment (MRG3-prior patient account number)
...
// with one account identifier (PID-18-account number) has been moved to another account identifier.
type ADT_A45 struct {
...
// Change external ID
//
// An A46 event is used to signal a change of an incorrectly assigned external ID value.  The “incorrect source external ID”
// value is stored in the MRG segment (MRG-4-prior patient ID-external) and is to be changed to the “correct target external
// ID” value stored in the PID segment (PID-2-patient ID-external).
//
// A change has been done at the external identifier level.  That is, a PID-2-patient ID (external ID) has been found to be incorrect
...
// Change internal ID
//
// An A47 event is used to signal a change of an incorrectly assigned internal ID value.  The “incorrect source internal ID”
// value is stored in the MRG segment (MRG-1-prior patient ID-internal) and is to be changed to the “correct target external
// ID” value stored in the PID segment (PID-3-patient ID (internal ID).
//
// A change has been done at the internal identifier level.  That is, a PID-3-patient ID (internal ID) has been found to be incorrect
// and has been changed.
type ADT_A47 struct {
...
//
// A change has been done at the alternate patient identifier level.  That is, a PID-4-alternate patient ID has been found to
// be incorrect and has been changed.
type ADT_A48 struct {
...
// notification is not related to whether or not the account is paid in full.  EVN-2-date/time of event must contain the account
// end date.
type BAR_P06 struct {
...
//
//
// When the requesting application knows that it is requisitioning nonstock supplies, it may also send an RQ1 segment with
// the RQD if at least one field in RQ1 is known to the sending application.  This may be necessary in order for the receiving application
// to properly determine where to get these supplies.  This depends on the sophistication of the database of the receiving
// application, which may contain a history of requisitions from the sending application
type OMN_O01 struct {
...
//
// Stock supplies are, as the name suggests, stocked in the hospital in designated areas, such as the warehouse, Central Supply,
// Nursing floors, or Operating Room.
//
// When requisitioning stock supplies, the requesting application need only specify the information in the RQD segment.
// It is assumed that this is enough information for the application receiving to identify the item.  If the sending application
// is not aware whether the supply is stock, it may optionally send an RQ1 along with the RQD.  Typically in that case, the item
// is requested with a free text description.
type OMS_O01 struct {
...
//
// The trigger event for this message is any change to an order.  Such changes include submission of new orders, cancellations,
// updates, patient and nonpatient-specific orders, etc.
type ORM_O01 struct {
...
//
// The QRF and QRD segments from the QRY are echoed back in the response.  The DSC segment contains the continuation pointer,
...
// If the pharmacy or treatment application (or some other application) needs to create a nonambiguous MAR report where each
// administration is matched to a particular give date/time instruction, it may use the RGV message
type RGV_O01 struct {
...
//
// When a state system does not already have the complete patient vaccination record, it will send a query (with a V01 event)
// for the definitive (last updated) record.  Within the definitions for QRD and QRF, certain components are defined according
// to position in the field, as detailed in Section 4.11, “QUERIES FOR IMMUNIZATION RECORDS (QRF SEGMENTS).”
type VXQ_V01 struct {
...
//
// When a provider using one system wishes to update the patient’s vaccination record being held in another system, he will
// transmit an unsolicited update of the record (with a V04 event).
type VXU_V04 struct {
...
//
// In response to a query for the definitive patient vaccination record, the system holding the record will return it to the
// system originating the query
type VXX_V02 struct {
